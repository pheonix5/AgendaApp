...
import { Select, Center, Box, CheckIcon, FormControl, WarningOutlineIcon } from "native-base";
...
type UserAgendaProps = Omit<UserProps, 'avatar' | 'imageAdress' | 'adressSend' | 'confirmado'>
...
export default function Agendamento() {
  ...
  const DataCompleta = format(dia?.date?.toDate()!!, 'dd/MM/yyyy', { locale: ptBR });
  ...
  async function handleAgendar() {
    ...
    try {
      ...
      await firebase.firestore().collection('vehicles').doc(agendaAll?.idVehicle).update(updatedVehicle)
      Alert.alert('Agendamento realizado com sucesso!', 'Você precisa confirmar a sua presença na tela meu agendamento.',[
        {
          text: 'ok',
          onPress: () => navigation.reset({ index: 0, routes: [{ name: 'Agenda' }] })
  ...
  return (
    <SafeAreaView className='flex-1 px-6 pb-4' style={{ marginTop: Constants.statusBarHeight - 10 }}>
      ...
      <Center>
        <Box maxW="300">
          <FormControl w='3/4' maxW={300} isInvalid={invalidService} isReadOnly>
            ...
            <FormControl.ErrorMessage leftIcon={<WarningOutlineIcon size='xs' />}>Escolha um serviço!</FormControl.ErrorMessage>
          </FormControl>
        </Box>
      </Center>
      ...
      <View className='flex-1 justify-end'>
        <Button.Root variant='roudend1' className='gap-6' onPress={handleAgendar}>
          {loading ? (
