...
const userSchema = yup.object({
  ...
  cpf: yup.string().required('CPF é obrigatório').test('is-valid-cpf', 'CPF inválido', value => value ? validaCPF(value) : false)
...
export default function ConfirmaPerfil() {
  const { control, handleSubmit, watch, setError, formState: { errors, } } = useForm<UserFormDataProps>({
    mode: 'onChange',
  ...
  async function handleSaveProfile(data: UserFormDataProps) {
    ...
    if (userData) {
      await saveProfile(avatar, imagem, documentoUri, userData, dataSave, isDoc, docName, setUserData, setLoadingSend, )
    }
  }

  return (
    <View className='flex-1'>
      <ScrollView showsVerticalScrollIndicator={false} className='flex-1 px-6' contentContainerStyle={{ paddingVertical: 16 }}>
        ...
        <View className='pt-4 gap-2'>
          <Controller
            ...
            render={({ field: { onChange } }) => (
              <Input.Root erroMessage={errors.nome?.message}>
                <Input.Label label='Nome Completo' className='text-gray-800 font-medium' />
                <Input.Container erroMessage={errors.nome?.message} isFocused={isFocused.nome}>
                  <Input.Icon icon={<MaterialCommunityIcons name="format-text" size={20} color={theme.color.ligthGray[300]} />} />
                  <Input.Mask
                    type='custom'
                    options={{
                      mask: '*'.repeat(50)
          ...
          <Controller
            ...
            render={({ field: { onChange } }) => (
              <Input.Root erroMessage={errors.cep?.message}>
                <Input.Label label='CEP' className='text-gray-800 font-medium' />
                <Input.Container erroMessage={errors.cep?.message} isFocused={isFocused.cep}>
                  <Input.Icon icon={<Entypo name="location-pin" size={20} color={theme.color.ligthGray[300]} />} />
                  <Input.Mask
                    type='zip-code'
                    placeholder='CEP'
                    onChangeText={onChange}
          ...
          <Button.Root
            ...
            activeOpacity={0.7}
          >
            <Button.Text title='COMPROVANTE DE ENDEREÇO' className='text-white' />
            <Button.Icon icon={<MaterialCommunityIcons name="paperclip" size={20} color="#FFF" />} />
          </Button.Root>
          ...
          <Controller
            ...
            render={({ field: { onChange } }) => (
              <Input.Root erroMessage={errors.cpf?.message}>
                <Input.Label label='CPF' className='text-gray-800 font-medium' />
                <Input.Container erroMessage={errors.cpf?.message} isFocused={isFocused.cpf}>
                  <Input.Icon icon={<MaterialCommunityIcons name="format-text" size={20} color={theme.color.ligthGray[300]} />} />
                  <Input.Mask
                    type='cpf'
                    placeholder='CPF'
          ...
          <Controller
            ...
            render={({ field: { onChange } }) => (
              <Input.Root erroMessage={errors.celular?.message}>
                <Input.Label label='Número de Celular' className='text-gray-800 font-medium' />
                <Input.Container erroMessage={errors.celular?.message} isFocused={isFocused.celular}>
                  <Input.Icon icon={<Feather name="phone-call" size={18} color={theme.color.ligthGray[300]} />} />
                  <Input.Mask
                    type='cel-phone'
