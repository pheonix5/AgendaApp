...
export function ListHorarios({
  ...
  const DataCompleta = format(
    diaSelecionado?.date?.toDate() || new Date(),
    "dd/MM/yyyy",
    { locale: ptBR }
  );
  ...
  return (
    ...
    >
      <View className="w-ful px-4 h-12 flex-row items-center justify-between bg-Cblue-300">
        ...
        <Text className="text-base text-white font-titulo">
          {data.vagas - data.vagasOcupadas} VAGAS
        </Text>
      </View>

      <View className="flex-row justify-between items-center px-4 h-16">
        <Text className="text-lg font-titulo text-Cgray-300">
          Dia {DataCompleta}
        </Text>
