...
export function ModalPicker({ isAvatar, setImage, setAvatar, setDocumento, setDocName, setIsDoc, onClose }: ModalPickerProps) {
  const offset = useSharedValue(0)
  const pan = Gesture.Pan().shouldCancelWhenOutside(true).onChange((event) => {
      ...
      <GestureDetector gesture={pan}>
        ...
        >
          <View className='w-[50%] self-end flex-row justify-between mr-4 mt-1'>
            <MaterialCommunityIcons name='drag-horizontal' size={24} color={theme.color.ligthGray[100]} />
            <FontAwesome name='close' size={24} color={theme.color.ligthGray[100]} onPress={handleClose} />
          </View>

          <Text className='font-medium text-lg text-ligthGray-100  text-center'>Selecione o arquivo</Text>

          <View className='flex-1 w-full flex-row gap-6 justify-center'>

              <TouchableOpacity className='items-center justify-center' onPress={takePhoto}>
                <AntDesign name="camera" size={32} color={theme.color.ligthGray[100]} />
                <Text className='text-ligthGray-100 text-base font-medium'>Camera</Text>
              </TouchableOpacity>

              <TouchableOpacity className='items-center  justify-center' onPress={pickImage}>
                <Entypo name="image-inverted" size={32} color={theme.color.ligthGray[100]} />
                <Text className='text-ligthGray-100 text-base font-medium'>Galeria</Text>
              </TouchableOpacity>

              {!isAvatar && (
                <TouchableOpacity className='items-center text-base justify-center' onPress={takeDocument}>
                  <Ionicons name="document" size={32} color={theme.color.ligthGray[100]} />
                  <Text className='text-ligthGray-100 font-medium'>Documento</Text>
                </TouchableOpacity>
              )}
          </View>
        </Animated.View>
