...
type UserStoreProps = {
  ...
  getUserAsyncStorage: () => Promise<void>;
...
export const useUserStorage = create<UserStoreProps>((set, get) => ({
    ...
    if (tipoAcao == 'cadastro') {
      await firebase.auth().createUserWithEmailAndPassword(email, senha)
        .then(async (userCrendential) => {
          await firebase.firestore().collection('users').doc(userCrendential.user?.uid).set({
  ...
  login: async (email, senha) => {
    await firebase.auth().signInWithEmailAndPassword(email, senha)
      .then(async (userCrendential) => {
        await firebase.firestore().collection('users').doc(userCrendential.user?.uid).get()
          .then((snapshot) => {
